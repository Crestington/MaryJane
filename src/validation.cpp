...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!Consensus::CheckTxInputs(tx, state, m_view, m_active_chainstate.m_blockman.GetSpendHeight(m_view), ws.m_base_fees, args.m_chainparams.GetConsensus())) {
...
CAmount GetBlockValue(int nHeight, const CAmount& nFees)
{
    ...
    if (nHeight == 0) {
        ...
        // Premine: First 10 block are 10,000,000
        nSubsidy = 10000000 * COIN;
    } else if (nHeight < 60) {
        // Bonus reward for block under 80, 1 coin
        nSubsidy = 1 * COIN;
    }
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // Small optimization whilst syncing
    bool fScriptChecks = !IsInitialBlockDownload();
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        else
        {
            ...
            if (!Consensus::CheckTxInputs(tx, tx_state, view, pindex->nHeight, txfee, m_params.GetConsensus(), (pindex->pprev? pindex->pprev->nMoneySupply : 0))) {
...
double GetInflationAdjustment(CChainState* active_chainstate, const Consensus::Params& consensusParams)
{
    ...
    LogPrint(BCLog::BENCH,  "- Inflation Unbound Adjustment = %s Using last %s blocks from height %s to height %s: %.2fms\n", nRawInflationAdjustment, nBlocksPerMonth, nHeightPrev, pindex->nHeight, nTime * 0.001);
