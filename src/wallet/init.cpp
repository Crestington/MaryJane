...
void WalletInit::AddWalletOptions(ArgsManager& argsman) const
{
    ...
    argsman.AddArg("-hdseed=<seed>", "HD Seed.", ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    argsman.AddArg("-mnemonic=<mnemonic>", "HD Seed.", ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    argsman.AddArg("-mnemonicpassphrase=<passphrase>", "HD Seed.", ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
    ...
    argsman.AddArg("-reservebalance=<amt>", strprintf("Reserved balance amount (default: %s)",
                                                            FormatMoney(DEFAULT_RESERVEBALANCE)), ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
...
#if HAVE_SYSTEM
    argsman.AddArg("-stakenotify=<cmd>", "Execute command when a coinstake received. %s in cmd is replaced by TxID, %w is replaced by wallet name, %b is replaced by the hash of the block including the transaction (set to 'unconfirmed' if the transaction is not included) and %h is replaced by the block height (-1 if not included). %w is not currently implemented on windows. On systems where %w is supported, it should NOT be quoted because this would break shell escaping used to invoke the command.", ArgsManager::ALLOW_ANY, OptionsCategory::WALLET);
